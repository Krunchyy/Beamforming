...
public class BeamFormingMaxSNR extends AbstractOrdonnanceur {
	...
	public BeamFormingMaxSNR(ConcurrentHashMap<IModel, Queue<Packet>> map, OFDM ofdm) {
		super(map, ofdm);
	...
	private Mobile getMobileWithBestSNR(int timeslot, int subcarrier, ArrayList<Mobile> emptyBuffersMobile) {
		...
		for (Iterator<Entry<IModel, Queue<Packet>>> iterator = entry.iterator(); iterator.hasNext();){
			...
			if(((Mobile) iter.getKey()).isBeamforming()) {
				BeamSubCarrier beam = ((Mobile) iter.getKey()).getBeamSubCarrier(this.agent, timeslot);
				
				if(beam.getSubCarrier() == subcarrier) {
					SNR = beam.getMkn();
